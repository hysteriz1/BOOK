...
type UserQuery struct {
	...
	// eager-loading edges.
	withUSERBOOK       *BookQuery
	withUSERUSERSTATUS *UserStatusQuery
	withFKs            bool
...
// QueryUSERBOOK chains the current query on the USER_BOOK edge.
func (uq *UserQuery) QueryUSERBOOK() *BookQuery {
...
// QueryUSERUSERSTATUS chains the current query on the USER_USERSTATUS edge.
func (uq *UserQuery) QueryUSERUSERSTATUS() *UserStatusQuery {
...
//  WithUSERBOOK tells the query-builder to eager-loads the nodes that are connected to
// the "USER_BOOK" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithUSERBOOK(opts ...func(*BookQuery)) *UserQuery {
	query := &BookQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUSERBOOK = query
	return uq
}

//  WithUSERUSERSTATUS tells the query-builder to eager-loads the nodes that are connected to
// the "USER_USERSTATUS" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithUSERUSERSTATUS(opts ...func(*UserStatusQuery)) *UserQuery {
	query := &UserStatusQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUSERUSERSTATUS = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		loadedTypes = [2]bool{
			uq.withUSERBOOK != nil,
			uq.withUSERUSERSTATUS != nil,
		}
	)
	if uq.withUSERUSERSTATUS != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := uq.withUSERBOOK; query != nil {
		...
		query.Where(predicate.Book(func(s *sql.Selector) {
			s.Where(sql.InValues(user.USERBOOKColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_user_book
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_book" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_book" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.USERBOOK = append(node.Edges.USERBOOK, n)
		}
	}

	if query := uq.withUSERUSERSTATUS; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if fk := nodes[i].user_status_userstatus_user; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_status_userstatus_user" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.USERUSERSTATUS = n
			}
		}
	}
