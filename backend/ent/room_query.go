...
type RoomQuery struct {
	...
	// eager-loading edges.
	withROOMBOOK     *BookQuery
	withROOMROOMTYPE *RoomTypeQuery
...
// QueryROOMBOOK chains the current query on the ROOM_BOOK edge.
func (rq *RoomQuery) QueryROOMBOOK() *BookQuery {
	query := &BookQuery{config: rq.config}
...
// QueryROOMROOMTYPE chains the current query on the ROOM_ROOMTYPE edge.
func (rq *RoomQuery) QueryROOMROOMTYPE() *RoomTypeQuery {
...
// QueryROOMSTATUS chains the current query on the ROOM_STATUS edge.
...
//  WithROOMBOOK tells the query-builder to eager-loads the nodes that are connected to
// the "ROOM_BOOK" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RoomQuery) WithROOMBOOK(opts ...func(*BookQuery)) *RoomQuery {
	query := &BookQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withROOMBOOK = query
	return rq
}

//  WithROOMROOMTYPE tells the query-builder to eager-loads the nodes that are connected to
// the "ROOM_ROOMTYPE" edge. The optional arguments used to configure the query builder of the edge.
func (rq *RoomQuery) WithROOMROOMTYPE(opts ...func(*RoomTypeQuery)) *RoomQuery {
	query := &RoomTypeQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	rq.withROOMROOMTYPE = query
	return rq
}

//  WithROOMSTATUS tells the query-builder to eager-loads the nodes that are connected to
// the "ROOM_STATUS" edge. The optional arguments used to configure the query builder of the edge.
...
// the "ROOM_INFO" edge. The optional arguments used to configure the query builder of the edge.
...
func (rq *RoomQuery) sqlAll(ctx context.Context) ([]*Room, error) {
	var (
		...
		loadedTypes = [4]bool{
			rq.withROOMBOOK != nil,
			rq.withROOMROOMTYPE != nil,
			rq.withROOMSTATUS != nil,
			rq.withROOMINFO != nil,
		}
	)
	if rq.withROOMROOMTYPE != nil || rq.withROOMSTATUS != nil || rq.withROOMINFO != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withROOMBOOK; query != nil {
		...
		query.Where(predicate.Book(func(s *sql.Selector) {
			s.Where(sql.InValues(room.ROOMBOOKColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.room_room_book
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "room_room_book" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_room_book" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ROOMBOOK = append(node.Edges.ROOMBOOK, n)
		}
	}

	if query := rq.withROOMROOMTYPE; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].room_type_roomtype_room; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_type_roomtype_room" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ROOMROOMTYPE = n
			}
		}
	}
	...
	if query := rq.withROOMSTATUS; query != nil {
		...
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].room_status_status_room; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_status_status_room" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := rq.withROOMINFO; query != nil {
		...
		nodeids := make(map[int][]*Room)
		for i := range nodes {
			if fk := nodes[i].room_info_info_room; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "room_info_info_room" returned %v`, n.ID)
			}
			for i := range nodes {
