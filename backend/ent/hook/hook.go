...
// The BookstatusFunc type is an adapter to allow the use of ordinary
// function as Bookstatus mutator.
type BookstatusFunc func(context.Context, *ent.BookstatusMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f BookstatusFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.BookstatusMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.BookstatusMutation", m)
	}
	return f(ctx, mv)
}
