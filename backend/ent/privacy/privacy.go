...
// The BookstatusQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type BookstatusQueryRuleFunc func(context.Context, *ent.BookstatusQuery) error

// EvalQuery return f(ctx, q).
func (f BookstatusQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.BookstatusQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.BookstatusQuery", q)
}

// The BookstatusMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type BookstatusMutationRuleFunc func(context.Context, *ent.BookstatusMutation) error

// EvalMutation calls f(ctx, m).
func (f BookstatusMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.BookstatusMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.BookstatusMutation", m)
}
